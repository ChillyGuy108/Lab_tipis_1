...
def get_spectre(arr: npt.NDArray[Any]):
    spec=np.abs(np.fft.rfft(arr)) / len(arr)
    spec[1:] = 2.0 * spec[1:]
    return spec
...
for i, f in enumerate(frequencies):
    ...
    spec_harm=get_spectre(sig_harm)
    ...
    spec_meander=get_spectre(sig_meander)
...
plt.tight_layout()
plt.savefig('signals_and_spectra_fixed_lines3.png', dpi=300)
plt.show()
